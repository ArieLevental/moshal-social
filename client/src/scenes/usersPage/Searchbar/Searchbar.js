...
const Searchbar = ({ institutionsData, companiesData }) => {
    ...
    setPresentedData(
      usersDbData.filter((user) => {
        return (
          ...
          (companyFilter ? user.occupation.some((occupation) => occupation.companyId === companyFilter) : true) &&
          (universityFilter ? user.education.some((education) => education.institutionId === universityFilter) : true)
        )
      })
    )
  }
  ...
  const searchUpdateHandler = (event) => {
    ...
    activateFilter(event.target.value.toLowerCase(), companyFilter, universityFilter)
  }
  ...
  const randomMoshalnikHandler = (event) => {
    setPresentedData(Array(usersDbData[Math.floor(Math.random() * usersDbData.length)]))
  }
  ...
  return (
    <div className='search-bar'>
      ...
      <div className='filter-zone'>
        ...
        <select id='companyId' name='companyId' onChange={searchCompanyHandler} defaultValue='none'>
          <option value=''></option>
        ...
        <select id='institutionId' name='institutionId' onChange={searchUniversityHandler} defaultValue='none'>
          <option value=''></option>
